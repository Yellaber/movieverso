...
@Component({
  ...
  template: `
    @if(title() && backdropImagePath() && backdropImageSrcset()) {
      <img class="md:max-w-full object-cover object-center rounded-sm" [ngSrc]="backdropImagePath()" [ngSrcset]="backdropImageSrcset()"
      sizes="(min-width: 768px) 50vw, 80vw" width="640" height="360" [alt]="title()"/>
    }
  `,
