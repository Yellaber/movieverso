...
export class CarouselService {
  ...
  private totalScrollStep = computed<number>(() =>
    this.totalCards() * this.cardContainerWidth() - this.scrollVisibleCards()
  );
  private getPartNotVisible = computed<number>(() =>
    (this.visibleCards() - Math.floor(this.visibleCards())) * this.cardContainerWidth()
  );
  getScrollStep = computed(() => this.scrollStep());
  isPrevious = computed<boolean>(() => this.scrollStep() > 0);
  isNext = computed<boolean>(() => this.scrollStep() < this.totalScrollStep() - this.getPartNotVisible() - GAP);
  ...
  next() {
    ...
    const max = this.totalScrollStep() - this.getPartNotVisible();
